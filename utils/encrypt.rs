...
fn load_config() -> Result<AppConfig, String> {
    ...
    let config: AppConfig = serde_yaml_ng::from_str(&config_content)
...
fn encrypt_uuid(uuid_str: &str, key_base64: &str) -> Result<String, String> {
    ...
    Ok(BASE64.encode(&encrypted_data).replace('+', "-").replace('/', "_").replace('=', ""))
}
...
// 写入加密结果到文件
fn write_results_to_file(results: &[EncryptionResult], input_file_path: &str, config: &AppConfig) -> Result<(), String> {
    // 生成输出文件名
