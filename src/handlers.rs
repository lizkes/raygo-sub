...
fn decrypt_secret(encrypted_secret: &str, key_base64: &str) -> Result<String, String> {
    ...
    if key_bytes.len() != 32 {
        return Err(format!("密钥长度不正确，需要32字节，实际: {} 字节", key_bytes.len()));
    }
    ...
    let normalized_data = encrypted_secret
        .replace('-', "+")
        .replace('_', "/");
    ...
    let encrypted_bytes = BASE64.decode(&padded_data)
    ...
    String::from_utf8(plaintext)
        .map_err(|e| format!("解密结果不是有效的UTF-8字符串: {}", e))
}
    ...
    if let Some(ref mut proxies) = clash_config.proxies {
        ...
        for proxy in proxies.iter_mut() {
            if proxy.contains_key("uuid") {
                proxy.insert("uuid".to_string(), serde_yaml_ng::Value::String(uuid.to_string()));
                replaced_count += 1;
            }
        }
        debug!("[{}] 🔄 替换了 {} 个代理的UUID为: {}", client_ip, replaced_count, uuid);
    } else {
    ...
    if use_compression {
        ...
        let compressed_data = match zstd::encode_all(yaml_body.as_bytes(), 3) {
            Ok(data) => {
                ...
                info!("[{}] ✅ 成功生成压缩订阅配置，原始大小: {} 字节，压缩后大小: {} 字节，压缩率: {:.1}%", 
        ...
        HttpResponse::Ok()
            ...
            .header("Content-Disposition", "attachment; filename=RayGo; filename*=UTF-8''RayGo%E8%AE%A2%E9%98%85")
            .header("Cache-Control", "no-cache")
            .header("X-Original-Size", original_size.to_string())
            .body(compressed_data)
    } else {
        // 返回未压缩的响应
        info!("[{}] ✅ 成功生成订阅配置，大小: {} 字节", client_ip, original_size);
